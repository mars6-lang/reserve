...
@section('content')
    ...
    <div class="max-w-5xl mx-auto px-4 py-6">
        ...
        @forelse ($orders as $order)
            ...
            @if ($order->status === 'active')
                <form action="{{ route('seller.markCompleted', $order->id) }}" method="POST" class="mt-3">
                    @csrf
                    @method('PATCH')
                    <button type="submit" class="px-4 py-1 bg-green-500 text-white text-sm rounded hover:bg-green-600">
                        Mark as Completed
                    </button>
                </form>
            @endif
